...
# with open("edu_fineweb10B_megatron/demo_text_document.idx", "rb") as f:
#     content = f.read()
#     print(content)
# exit()
...
class Partition(object):
    ...
    def process_npy_file(self, file_name):
        input_file_name, output_prefix = file_name
        print("Opening", input_file_name)
        #fin = open(input_file_name, 'r', encoding='utf-8')
        fin = np.load(input_file_name)
        ...
        total_bytes_processed = 0
        print("Time to startup:", startup_end - startup_start)
        total_bytes_processed += len(fin)
        builders[key].add_document(fin, [len(fin)])
        self.print_processing_stats(1, proc_start, total_bytes_processed)
...
def main():
    ...
    for name in in_ss_out_names:
        p = multiprocessing.Process(target=partition.process_npy_file,
